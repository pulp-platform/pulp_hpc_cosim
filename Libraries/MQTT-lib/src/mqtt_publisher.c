...
int publisher_init(struct mqtt_instance *val)
{
	...
	mosquitto_connect_callback_set(val->mosq, connect_callback); //This is called when the broker sends a CONNACK message in response to a connection.
	//mosquitto_message_callback_set(val->mosq, message_callback); //This is called when a message is received from the broker.
	mosquitto_publish_callback_set(val->mosq, publish_callback); //This is called when a message initiated with mosquitto_publish has been sent to the broker successfully.

    mosquitto_username_pw_set(val->mosq, val->username, val->passwd);

	while (mosquitto_connect(val->mosq, val->broker_ip, val->broker_port, MQTT_KEEPALIVE) != MOSQ_ERR_SUCCESS) {
...
{
	int rc = mosquitto_publish(val->mosq, &message->mid, message->topic, message->length, message->address, MQTT_QOS, false);
