...
int mySem_wait(mySem_t *mysem) {
    ...
    if (rc) {
        printf("pthread_mutex_lock failed (wait) - %d\n\r", rc);
        exit(1);
    }
    return_value = rc;
    ...
    if (rc) {
        printf("pthread_mutex_unlock failed (wait) - %d\n\r", rc);
        exit(1);
    }
    return_value = (return_value | rc);
...
int mySem_post(mySem_t *mysem) {
    ...
    rc = pthread_mutex_lock(&(mysem->mutex));
    if (rc) {
        printf("pthread_mutex_lock failed (post) - %d\n\r", rc);
        /*for (int i = 0; i < 4; ++i)
        {
                for (int j = 0; j < 40; ++j)
                {
                        printf("Thread: %d, Id: %d, nsec: %ld \n\r", i, j,
        threadtime[i][j].tv_nsec);
                }
                printf("\n\r");
        }*/
        exit(1);
    ...
    rc = pthread_mutex_unlock(&(mysem->mutex));
    if (rc) {
        printf("pthread_mutex_unlock failed (post) - %d\n\r", rc);
        exit(1);
    }
    return_value = (return_value | rc);
