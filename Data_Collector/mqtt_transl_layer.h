...
// #define MQTT_BROKER_IP "137.204.213.167" //"137.204.213.228" //"127.0.0.1" //"137.204.213.192"
// /228
#define MQTT_PORT 1883 // Default MQTT port

#define MQTT_MESSAGE_SIGNAL_INIT(MESS_SIGNAL) \
    {} //{mySem_t* val_sem_ = (mySem_t*)MESS_SIGNAL; mySem_init(val_sem_, 0, 0, mySem_Binary);}
#define MQTT_MESSAGE_SIGNAL_DEINIT(MESS_SIGNAL) \
    {} //{mySem_t* val_sem_ = (mySem_t*)MESS_SIGNAL; mySem_destroy(val_sem_);}
...
#define MQTT_SIGNAL_MESSAGE(MESS_SIGNAL) \
    {}                        //{mySem_t* val_sem_ = (mySem_t*)MESS_SIGNAL; mySem_post(val_sem_);}
#define MQTT_SIGNAL_PUBLISH() // mySem_post(&sem_mqtt_publish_done)

#define MQTT_AWAKE_ON_MESSAGE(MESS_SIGNAL) \
    {}                          //{mySem_t* val_sem_ = (mySem_t*)MESS_SIGNAL; mySem_wait(val_sem_);}
#define MQTT_AWAKE_ON_PUBLISH() // mySem_wait(&sem_mqtt_publish_done)

#define MQTT_LOCK_MESSAGE_INIT(LOCK) \
    { \
        int rc; \
        pthread_mutexattr_t lock_attr; \
        rc = pthread_mutexattr_init(&lock_attr); \
        if (rc) { \
            printf("Error attr for Mutex Init: %d\n\r", rc); \
        } \
...
#define MQTT_LOCK_MESSAGE_DEINIT(LOCK) \
    { \
        int rc; \
        pthread_mutex_t *val_lock_ = (pthread_mutex_t *)LOCK; \
...
#define MQTT_LOCK_MESSAGE_TAKE(LOCK) \
    { \
        int rc; \
        pthread_mutex_t *val_lock_ = (pthread_mutex_t *)LOCK; \
        rc = pthread_mutex_lock(val_lock_); \
        if (rc) { \
            printf("pthread_mutex_lock failed (message lock) - %d\n\r", rc); \
            exit(1); \
        } \
    }
#define MQTT_LOCK_MESSAGE_RELEASE(LOCK) \
    { \
        int rc; \
        pthread_mutex_t *val_lock_ = (pthread_mutex_t *)LOCK; \
        rc = pthread_mutex_unlock(val_lock_); \
        if (rc) { \
            printf("pthread_mutex_unlock failed (message lock) - %d\n\r", rc); \
            exit(1); \
        } \
    }
...
// prepare examon topic:
        ...
        if (test_num < 0.001f) {
            sprintf(examon_time, "%d.000%d", (epoch_time + report_add),
                    (int)(id * EXAMON_TIME_MUL - (report_add * 1000000)));
...
#ifdef USE_EXAMON
                // finish topic
                snprintf(mqtt_examon_message.topic, EXAMON_MAX_TOPIC_CHAR, "%s%d%s", examon_topic,
                         i, examon_final_name);
...
#ifdef USE_EXAMON
                // finish topic
                snprintf(mqtt_examon_message.topic, EXAMON_MAX_TOPIC_CHAR, "%s%d%s%d", examon_topic,
                         (i / WL_STATES), examon_final_name, (i % WL_STATES));
                // strcpy(mqtt_examon_message.topic, examon_topic);
                // printf("topic: %s\n\r", mqtt_examon_message.topic);
                char *examon_data_index = examon_data + i * 64 * sizeof(char);
                sprintf(examon_data_index, "%d;%s", message_addr[i], examon_time);
