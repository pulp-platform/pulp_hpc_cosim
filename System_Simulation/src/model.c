...
double *A = NULL;
double *B = NULL;
// had to put these here even though they are just needed inside model_step, cuz otherwise I needed
// them to be malloced each iteration
double *inputMat;
double *tempInput;
double *tempState;
    ...
    */
    ...
    for(int t=0; t<N_EPI_CORE*2*N_EPI_CORE*2; t++)
    {
        if (A[t] - A_test[t] > 1e-3)
            printf("A[%d][%d]: %f / %f \n\r", (t/(N_EPI_CORE*2)), (t%(N_EPI_CORE*2)), A[t],
    A_test[t]); if (B[t] - B_test[t] > 1e-3) printf("B[%d][%d]: %f / %f \n\r", (t/(N_EPI_CORE*2)),
    (t%(N_EPI_CORE*2)), B[t], B_test[t]);
    }
    */
...
void model_step(float *oTemp, float *oDomainPower, float (*iWlPerc)[WL_STATES], int sensor_read) {
    ...
    */
    ...
    for (int i = 0; i < simulation.nb_elements; i++) {
        ...
        // printf(" - pointers %d: %p, %p\n", i, &simulation.elements[i],
        // simulation.elements[i].compute_power);
        float power =
            simulation.elements[i].compute_power(&simulation.elements[i], ifp_ctrl_core_freq[i],
                                                 ifp_ctrl_quad_vdd[simulation.elements[i].domain],
                                                 (float)state[i * 2], 1.0, &iWlPerc[i][0]);
        // printf("number: %d, value: %f\n\r", i, power);
        // printf("freq: %f, vdd: %f, temp: %f, instr: %f %f %f %f %f\n\r", ifp_ctrl_core_freq[i],
        // ifp_ctrl_quad_vdd[CoreQuad[i]], state[i*2], iWlPerc[i][0], iWlPerc[i][1], iWlPerc[i][2],
        // iWlPerc[i][3], iWlPerc[i][4]);
        ...
        for (int i = 0; i < Nc; i++)
        {
                ...
                Pdin = (1 - throttl) * ( equiv_Ceff * ifp_ctrl_core_freq[i] *
ifp_ctrl_quad_vdd[CoreQuad[i]] * ifp_ctrl_quad_vdd[CoreQuad[i]] ) ; Pthrottl = throttl *
(P_thrott__);
        ...
        */
        ...
        if (simulation.elements[i].type == JT_CORE) // TOO fix thermal model
        {
            ...
            inputMat[core_count] =
                (double)(power * ThermalCorrection[core_count]); // after, because we want it to
                                                                 // affect just the thermal model
        ...
        // inputMat[i] = (double)(power * ThermalCorrection); //after, because we want it to affect
        // just the thermal model
    }
...
double spreading_r_computation(double source_area, double plate_area, double plate_thickness,
                               double k_source, double R_plate) {
    ...
    double phi_c =
        (tanh(delta_c * tau) + delta_c / Biot) / (1.0 + delta_c / Biot * tanh(delta_c * tau));

    double psi_avg = pow(1.0 - epsi, 1.5) * phi_c / 2.0;
    double psi_max = (1.0 - epsi) * phi_c / (pow(M_PI, 0.5));

    // printf("src: %f-%f, plt: %f-%f, tau: %f, epsi: %f, h: %f, Biot: %f, delta: %f, phi: %f, psi:
    // %f\n\r", src_r, source_area, plt_r, plate_area, tau, epsi, h, Biot, delta_c, phi_c, psi_avg);
...
int create_thermal_model(double **A_th, double **B_th, int Nc, int Nh, int Nv, float Ts) {
    ...
    if (Nc != Nh * Nv) {
        // TODO: Make a modification for 'not exactly symmetric' cores configurations: i.e. a 9 core
        // in 2x5 disposition
        printf("[MODEL] Error! The number of cores is not equal to the product of horizontal and "
               "vertical dimensions in cores disposition\r\n");
        return 1;
    }
    ...
    for (int i = 0; i < add_states; i++) A_comp[i] = wid_nFML[i] * len_nFML[i];
    ...
    double air_al_t =
        t_nFML[end - air_pos] - (t_nFML[end - al_pos] + t_cu + t_si + t_nFML[end - pcb_pos]);
    ...
    double Ri_air_al_len = (len_nFML[end - air_pos] - len_nFML[end - al_pos]) / 2.0 /
                           (wid_nFML[end - air_pos] * t_nFML[end - air_pos]) / k_air / air_factor;
    double Ri_air_al_wid = (wid_nFML[end - air_pos] - wid_nFML[end - al_pos]) / 2.0 /
                           (len_nFML[end - air_pos] * t_nFML[end - air_pos]) / k_air / air_factor;
    //
    double Ri_al_v = t_nFML[end - al_pos] / A_comp[end - al_pos] / k_al;
    double Ri_al_len =
        len_nFML[end - al_pos] / (wid_nFML[end - al_pos] * t_nFML[end - al_pos]) / k_al;
    double Ri_al_wid =
        wid_nFML[end - al_pos] / (len_nFML[end - al_pos] * t_nFML[end - al_pos]) / k_al;
    //
    ...
    h_alair_cond = k_air / (air_al_t + t_nFML[end - al_pos] / 2.0) *
                   pow((0.825 + (0.387 * pow(RaL, (1.0 / 6.0))) /
                                    pow(1.0 + pow(0.492 / 7.039E-01, (9.0 / 16.0)), 8.0 / 27.0)),
                       2.0);
    double R_alair_len_cond =
        1.0 / (h_alair_cond * (wid_nFML[end - al_pos] * t_nFML[end - al_pos]));
    double R_alair_wid_cond =
        1.0 / (h_alair_cond * (len_nFML[end - al_pos] * t_nFML[end - al_pos]));
    ...
    R_spr = spreading_r_computation((wid_nFML[end - al_pos] * t_nFML[end - al_pos]),
                                    ...
                                    len_nFML[end - air_pos], k_al, Ri_air_al_len);
    ...
    R_spr = spreading_r_computation(len_nFML[end - al_pos] * t_nFML[end - al_pos],
                                    ...
                                    wid_nFML[end - air_pos], k_al, Ri_air_al_wid);
    ...
    for (int r = extt_rows; r < (rows - extb_rows); r++) {
        for (int c = extl_cols; c < (cols - extr_cols); c++) {
            ...
            if (r == (extt_rows)) RC_fp_dim[(r * cols + c) + north_pos * fpdim] += 0.3e-3;
            if (r == (rows - extb_rows - 1))
                RC_fp_dim[(r * cols + c) + south_pos * fpdim] += 0.3e-3;

            // Eternal Mesh connection: cols
            if (c == (extl_cols)) RC_fp_dim[(r * cols + c) + west_pos * fpdim] += 0.3e-3;
            if (c == (cols - extr_cols - 1)) RC_fp_dim[(r * cols + c) + east_pos * fpdim] += 0.3e-3;
    ...
    */
    ...
    // State Matrix assuming each node is a second order sub-system (silicon and heater dynamics)
    // interacting with neighbours and T_amb
    ...
    for (int i = 0; i < full_model_layers * Nc; i++) {
        if ((i % full_model_layers) == 0) // For each core:
        {
            ...
            // Computing Core R
            double li = RC_fp_dim[(irow * cols + icol) + north_pos * fpdim] +
                        RC_fp_dim[(irow * cols + icol) + south_pos * fpdim];
            double wi = RC_fp_dim[(irow * cols + icol) + east_pos * fpdim] +
                        RC_fp_dim[(irow * cols + icol) + west_pos * fpdim];
            // Si:
            double R_Si = R_fp_material[(irow * cols + icol) + si_pos * fpdim];
            double Ri_si_hn =
                RC_fp_dim[(irow * cols + icol) + north_pos * fpdim] / (wi * t_si) / R_Si;
            double Ri_si_hs =
                RC_fp_dim[(irow * cols + icol) + south_pos * fpdim] / (wi * t_si) / R_Si;
            double Ri_si_he =
                RC_fp_dim[(irow * cols + icol) + east_pos * fpdim] / (li * t_si) / R_Si;
            double Ri_si_hw =
                RC_fp_dim[(irow * cols + icol) + west_pos * fpdim] / (li * t_si) / R_Si;
            //
            ...
            double Ri_cu_hn =
                RC_fp_dim[(irow * cols + icol) + north_pos * fpdim] / (wi * t_cu) / R_Cu;
            double Ri_cu_hs =
                RC_fp_dim[(irow * cols + icol) + south_pos * fpdim] / (wi * t_cu) / R_Cu;
            double Ri_cu_he =
                RC_fp_dim[(irow * cols + icol) + east_pos * fpdim] / (li * t_cu) / R_Cu;
            double Ri_cu_hw =
                RC_fp_dim[(irow * cols + icol) + west_pos * fpdim] / (li * t_cu) / R_Cu;
            //
            ...
            wi = RC_fp_dim[((irow - 1) * cols + icol) + east_pos * fpdim] +
                 RC_fp_dim[((irow - 1) * cols + icol) + west_pos * fpdim];
            double Rin_si_hn = RC_fp_dim[((irow - 1) * cols + icol) + south_pos * fpdim] /
                               (wi * t_si) /
                               R_fp_material[((irow - 1) * cols + icol) + si_pos * fpdim];
            double Rin_cu_hn = RC_fp_dim[((irow - 1) * cols + icol) + south_pos * fpdim] /
                               (wi * t_cu) /
                               R_fp_material[((irow - 1) * cols + icol) + cu_pos * fpdim];
            wi = RC_fp_dim[((irow + 1) * cols + icol) + east_pos * fpdim] +
                 RC_fp_dim[((irow + 1) * cols + icol) + west_pos * fpdim];
            double Rin_si_hs = RC_fp_dim[((irow + 1) * cols + icol) + north_pos * fpdim] /
                               (wi * t_si) /
                               R_fp_material[((irow + 1) * cols + icol) + si_pos * fpdim];
            double Rin_cu_hs = RC_fp_dim[((irow + 1) * cols + icol) + north_pos * fpdim] /
                               (wi * t_cu) /
                               R_fp_material[((irow + 1) * cols + icol) + cu_pos * fpdim];
            li = RC_fp_dim[(irow * cols + (icol + 1)) + north_pos * fpdim] +
                 RC_fp_dim[(irow * cols + (icol + 1)) + south_pos * fpdim];
            double Rin_si_he = RC_fp_dim[(irow * cols + (icol + 1)) + west_pos * fpdim] /
                               (li * t_si) /
                               R_fp_material[(irow * cols + (icol + 1)) + si_pos * fpdim];
            double Rin_cu_he = RC_fp_dim[(irow * cols + (icol + 1)) + west_pos * fpdim] /
                               (li * t_cu) /
                               R_fp_material[(irow * cols + (icol + 1)) + cu_pos * fpdim];
            li = RC_fp_dim[(irow * cols + (icol - 1)) + north_pos * fpdim] +
                 RC_fp_dim[(irow * cols + (icol - 1)) + south_pos * fpdim];
            double Rin_si_hw = RC_fp_dim[(irow * cols + (icol - 1)) + east_pos * fpdim] /
                               (li * t_si) /
                               R_fp_material[(irow * cols + (icol - 1)) + si_pos * fpdim];
            double Rin_cu_hw = RC_fp_dim[(irow * cols + (icol - 1)) + east_pos * fpdim] /
                               (li * t_cu) /
                               R_fp_material[(irow * cols + (icol - 1)) + cu_pos * fpdim];
        ...
        if ((i % full_model_layers) == 0) {
            A_th_c[i * sdim + i] =
                -1.0 / (R_sicu_v * C_si) -
                1.0 / C_si * (1.0 / R_si_he + 1.0 / R_si_hn + 1.0 / R_si_hw + 1.0 / R_si_hs) -
                1.0 / (C_si * R_sipcb_v) * si_pcb_fact;
        ...
        {
            A_th_c[i * sdim + i] =
                -1.0 / (R_sicu_v * C_cu) - 1.0 / (R_cual_v * C_cu) -
                1.0 / C_cu * (1.0 / R_cu_he + 1.0 / R_cu_hn + 1.0 / R_cu_hw + 1.0 / R_cu_hs);
    ...
    // HeatSink
    A_th_c[(end - al_pos) * sdim + (end - air_pos)] =
        1.0 / C_al * (1.0 / R_alair_v + 2.0 / R_alair_len + 2.0 / R_alair_wid) + al_fan_dis / C_al;
    A_th_c[(end - air_pos) * sdim + (end - al_pos)] =
        1.0 / C_air * (1.0 / R_alair_v + 2.0 / R_alair_len + 2.0 / R_alair_wid) +
        al_fan_dis / C_al; // here C_al or C_air?
    A_th_c[(end - al_pos) * sdim + (end - al_pos)] = 0;
    double accum = 0;
    for (int i = 0; i < sdim; i++) accum += A_th_c[(end - al_pos) * sdim + i];
    ...
    accum = 0;
    for (int i = 0; i < sdim; i++) accum += A_th_c[(end - pcb_pos) * sdim + i];
    ...
    accum = 0;
    for (int i = 0; i < sdim; i++) accum += A_th_c[(end - mb_pos) * sdim + i];
    ...
    accum = 0;
    for (int i = 0; i < sdim; i++) accum += A_th_c[(end - air_pos) * sdim + i];
    A_th_c[(end - air_pos) * sdim + (end - air_pos)] =
        -accum - case_fan_dis; //-sum(A(end-lAirPos,:)) - case_fan_dis; // -0.149075;
    ...
    */
    ...
    for (int i = 0; i < 2 * Nc; i++) {
        if ((i + 1) % 2 > 0) {
            ...
            double li = CPw_fp_dim[(irow * core_cols + icol) + north_pos * fpldim] +
                        CPw_fp_dim[(irow * core_cols + icol) + south_pos * fpldim];
            double wi = CPw_fp_dim[(irow * core_cols + icol) + east_pos * fpldim] +
                        CPw_fp_dim[(irow * core_cols + icol) + west_pos * fpldim];
            double C_core =
                C_fp_material[((irow + extt_rows) * cols + (icol + extl_cols)) + si_pos * fpdim] *
                li * wi * t_si;
            // printf("mat: %lf, li: %lf, wi: %lf, t: %lf\n\r",
            // C_fp_material[((irow+extt_rows)*cols+(icol+extl_cols)) + si_pos*fpdim], li, wi,
            // t_si);
            // TODO: Noise
            /*
            if pdev==1
    ...
    for (int i = 0; i < udim; i++) B_th_c[(end - air_pos) * udim + i] = 0;
...
void generateGaussianNoise(float *z0, float *z1, float mu, float sigma) {
    ...
    if (z0 != NULL) *z0 = mag * (float)cos(2 * M_PI * u2) + mu;
    if (z1 != NULL) *z1 = mag * (float)sin(2 * M_PI * u2) + mu;
