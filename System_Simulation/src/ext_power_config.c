...
extpw_data_t ext_power_input[N_EXT_DOMAIN][N_EXT_PW_SEQ] = {
    {{15, 200000}, {11, 250000}, {7, 100000}, {12, 150000}, {13, 100000}},
...
float total_ext_power(int us_per_step) {
    if (us_counter[0] * 1000 / steps_per_sim_time_ms >=
        ext_power_input[0][seq_counter[0]].time_us) {
        seq_counter[0]++;
    ...
    // 	// I cannot do just the sum and divide for us_per_step*N_EXT_DOMAIN outside the for loop
    // because of
