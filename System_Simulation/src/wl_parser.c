...
void parse_wl(void) {
    ...
    printf("nc: %d ncols: %d, res: -%d / %d\n", n_cores, nb_cols, tr_base_cols,
           (tr_st_cols + tr_nd_cols));
    ...
    // int64_t* tr_li_data = (int64_t*)malloc(sizeof(int64_t)*n_cores*nb_rows*WL_STATES); //TODO
    // Check this! is it nb_* or simulation_elements..?
    ...
    while (row = CsvReadNextRow(handle)) {
        ...
        counter_rows+1, idx, load[idx], 1/cpi[idx], fpdp_cycles[idx*fp_dp_vect_dim+0],
        fpsp_cycles[idx*fp_dp_vect_dim+0], mem_cycles[idx], 0 );
        }
        */
        ...
        for (int idx = 0; idx < n_cores; idx++) {
            idle_c[idx] = (tot_cycles[idx] * (1 * aprx - (int64_t)(load[idx] * aprxf))) /
                          (int64_t)(load[idx] * aprxf);
            int_c[idx] = 0;
            float_c[idx] =
                (fpsp_cycles[idx * fp_sp_vect_dim + 0] * (int64_t)(cpi[idx] * fpsp_p * aprxf)) /
                    aprx +
                (fpdp_cycles[idx * fp_sp_vect_dim + 0] * (int64_t)(cpi[idx] * aprxf)) / aprx;
            int_c[idx] += (fpsp_cycles[idx * fp_sp_vect_dim + 0] *
                           (int64_t)(cpi[idx] * (1 - fpsp_p) * aprxf)) /
                          aprx;
            ...
            for (int idx2 = 1; idx2 < fp_sp_vect_dim; idx2++) {
                vect_c[idx] += (fpsp_cycles[idx * fp_sp_vect_dim + idx2] *
                                (int64_t)(cpi[idx] * vectsp_p[idx2 - 1] * aprxf)) /
                               aprx;
                float_c[idx] += (fpsp_cycles[idx * fp_sp_vect_dim + idx2] *
                                 (int64_t)(cpi[idx] * (1 - vectsp_p[idx2 - 1]) * aprxf)) /
                                aprx;
            }
            for (int idx2 = 1; idx2 < fp_dp_vect_dim; idx2++) {
                vect_c[idx] += (fpdp_cycles[idx * fp_dp_vect_dim + idx2] *
                                (int64_t)(cpi[idx] * vectdp_p[idx2 - 1] * aprxf)) /
                               aprx;
                float_c[idx] += (fpdp_cycles[idx * fp_dp_vect_dim + idx2] *
                                 (int64_t)(cpi[idx] * (1 - vectdp_p[idx2 - 1]) * aprxf)) /
                                aprx;
            }
            ...
            idx, idle_c[idx]/aprx, int_c[idx]/aprx, float_c[idx]/aprx, l2mem_c[idx]/aprx,
            vect_c[idx]/aprx );
            */
        }
