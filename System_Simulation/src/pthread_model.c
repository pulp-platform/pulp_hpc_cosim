...
#define MAX_MEAN_WINDOW \
    50 * (steps_per_sim_time_ms / 2) // TODO here should be steps_per_controller_iteration
...
/* SCMI */
    ...
    float *hlc_workload_accum =
        (float *)calloc(scmi_states_num * simulation.nb_cores, sizeof(float));
...
/*** Init ***/
    ...
    for (int i = 0; i < N_EPI_CORE; i++) {
        for (int state = 0; state < WL_STATES; state++) l_workload_mean[i][state] = 0;
    }
...
/* SCMI */
            ...
            for (int i = 0; i < simulation.nb_elements; i++) {
                generate_wl(simulation.elements[i].core_config.workload, ifp_ctrl_core_freq[i], i,
                            &simulation.elements[i]);
                cycles_per_step[i] =
                    (float)(ifp_ctrl_core_freq[i] * 1000000.0f) / steps_per_sim_time_ms;
/*
if (i == 0){
printf("------- Resulting Workload ------ \t --> Left: %ld - %f\n\r\t",
        current_quantum[i]->dim - current_quantum[i]->consumed, current_quantum[i]->us_wait_time -
current_quantum[i]->us_waited_time); for(int s=0;
s<get_component_states_number(&simulation.elements[i]); s++) printf("[%d]=%f , ", s,
simulation.elements[i].core_config.workload[s]); printf("\n\r");
}
*/
...
/* SCMI */
                ...
                for (int state = 0; state < scmi_states_num; state++) {
                    hlc_workload_accum[i * l_states_num + state] +=
                        simulation.elements[i].core_config.workload[state];

                    if (scmi_counter <= 0) {
                        hlc_workload_delivery[i * l_states_num + state] =
                            hlc_workload_accum[i * l_states_num + state] / (float)scmi_update_freq;
            ...
            if (*gs_workload_acc_read) {
                *gs_workload_acc_read = 0;
                for (int core = 0; core < simulation.nb_elements; core++) {
                    wl_accum_cycles[core] = (uint32_t)cycles_per_step[core] /
                                            CYCLYES_DIVIDER; // /10 we lose precision but we
                                                             // increase time before overflow

                    for (int state = 0; state < WL_STATES; state++) {
                        // float app = (l_workload_mean[core][state] / (float)wl_accum_cycles[core])
                        // * 100.0f; otp_instructions_information[core*WL_STATES + state] =
                        // (unsigned int)app; l_workload_mean[core][state] = 0.0f;
                        // wl_accum_cycles[core] = 0;

                        // l_workload_mean[core][state] =
                        // ((uint32_t)round(workload_perc[core][state]*100.0f)); // /
                        // cycles_per_step[core];
                        l_workload_mean[core][state] =
                            simulation.elements[core].core_config.workload[state] *
                            100.0f; // / cycles_per_step[core];
                    }
                }
            } else {
                mean_accum_counter++; // TBD: I can improve this and make it per core with a
                                      // different window / a different offset start?
                ...
                for (int core = 0; core < simulation.nb_elements; core++) {
                    ...
                    if ((mean_accum_counter > MAX_MEAN_WINDOW + 2) ||
                        (mean_accum_counter ==
                         0)) // + 2 to offset the window otherwise first value was no mean
                    {
                        wl_accum_cycles[core] = 0;
                        for (int state = 0; state < WL_STATES; state++)
                            l_workload_mean[core][state] = 0; // hard reset but no other way
                    ...
                    for (int state = 0; state < WL_STATES; state++) {
                        ...
                        //( ((uint32_t)round(workload_perc[core][state]*100.0f) *
                        //cycles_per_step[core]) - (l_workload_mean[core][state] *
                        //cycles_per_step[core]) )
                        float value =
                            (((simulation.elements[core].core_config.workload[state] * 100.0f) -
                              l_workload_mean[core][state]) *
                             (float)cycles_per_step[core] / (float)CYCLYES_DIVIDER) /
                        ...
                        otp_instructions_information[core * WL_STATES + state] =
                            (uint32_t)round(l_workload_mean[core][state]);
#else
                        // TODO: This is a nonsense, I should multiply to the Ceff, not divided for
                        // wl_accum_cycles. Fix this!
                        single_wl_accum +=
                            (uint32_t)(l_workload_mean[core][state]); // / wl_accum_cycles[core]);
#endif
                    }
#ifndef USE_INSTRUCTIONS_COMPOSITION
...
void *pthread_model_execution(void *ptr) {
    ...
    int *power_meas_precision_steps = (int *)malloc(
        sizeof(int) * (simulation.nb_power_domains + 1)); // = precision_ms * steps_per_sim_time_ms
    unsigned int *power_domain_step_counter =
        (unsigned int *)malloc(sizeof(unsigned int) * (simulation.nb_power_domains + 1));
    float *model_step_domain_pw =
        (float *)malloc(sizeof(float) * (simulation.nb_power_domains + 1));
#ifndef USE_MYSEM
    int sem_value = 0;
#endif
    int ret = 0;
    ...
    // power_meas_steps_offset[N_QUADS + 1] = {0, 0, 0}; //To be fixed, cuz if value !=0, the first
    // power value will be wrong.
...
/* Initialization */
    ...
    uint32_t core_thermal_sensor_update =
        (uint32_t)round((float)core_thermal_sensor_update_us / (1000.0f / steps_per_sim_time_ms));

    for (int core = 0; core < N_EPI_CORE; core++) {
        for (int state = 0; state < WL_STATES; state++) workload_perc[core][state] = 0;
    ...
    for (int i = 0; i < (gtl_nb_power_domains + 1); i++) {
        ...
        power_meas_precision_steps[i] =
            (int)round((float)power_meas_precision_us[i] * (float)steps_per_sim_time_ms / 1000.0f);
    }
...
// sem here, activated through timer
        ...
        if (!(*gn_pause_simulation)) {
            ...
            CycleIDnumber++;
            if (CycleIDnumber >= 1000000000) CycleIDnumber = 0;
...
// Debug printf
            ...
            ret = pthread_mutex_lock(&pthread_lock_printf);
            if (ret) printf("Pf Mutex Error: %s\n", strerror(ret));
            printf("MODEL:  Temp: %f,   WL: %f  %f  %f  %f,   PwB: %f\n\r", otp_model_core_temp[1],
                   workload_perc[1][0], workload_perc[1][1], workload_perc[1][2],
                   workload_perc[1][3], model_step_domain_pw[1]);
            ret = pthread_mutex_unlock(&pthread_lock_printf);
            if (ret) printf("Pf Mutex Error: %s\n", strerror(ret));
#endif
