...
void init_os(void) {
    ...
    if (nb_cols != simulation.nb_elements) {
        printf("[CSV] Error: Target Frequency file has a different number of columns (%d) wrt the "
               "components in the Model (%d)\r\n",
    ...
    TargetFrequency =
        (input_data_t *)malloc(sizeof(input_data_t) * nb_cols *
                               nb_rows); // TODO Check this! is it nb_* or simulation_elements..?
    N_FREQ_SEQ = nb_rows;
    // printf("freq: %d\n", nb_rows);
    ...
    while (row = CsvReadNextRow(handle)) {
        ...
        while (col = CsvReadNextCol(row, handle)) {
            /* col = CSV col string */
            TargetFrequency[counter_cols * nb_rows + counter_rows].time =
                time; // TODO Check this! is it nb_* or simulation_elements..?
                      // printf("%d/%d: %d\n", counter_rows, counter_cols,
                      // TargetFrequency[counter_cols*nb_rows + counter_rows].time);
            TargetFrequency[counter_cols * nb_rows + counter_rows].value =
                atof(col); // TODO Check this! is it nb_* or simulation_elements..?
            // printf("%d/%d: %f\n", counter_rows, counter_cols,
            // TargetFrequency[counter_cols*nb_rows + counter_rows].value);
    ...
    if (nb_cols != simulation.nb_power_domains) {
        printf("[CSV] Error: Power Budget file has a different number of columns (%d) wrt the "
               "power domains in the Model (%d)\r\n",
    ...
    BoardPwrBudget = (input_data_t *)malloc(
        sizeof(input_data_t) * nb_rows); // TODO Check this! is it nb_* or simulation_elements..?
    QuadPwrBudget =
        (input_data_t *)malloc(sizeof(input_data_t) * nb_rows *
                               nb_cols); // TODO Check this! is it nb_* or simulation_elements..?
    ...
    while (row = CsvReadNextRow(handle)) {
        ...
        while (col = CsvReadNextCol(row, handle)) {
            /* col = CSV col string */
            QuadPwrBudget[counter_cols * nb_rows + counter_rows].time =
                time; // TODO Check this! is it nb_* or simulation_elements..?
            // printf("%d/%d: %d\n", counter_rows, counter_cols, QuadPwrBudget[counter_cols*nb_rows
            // + counter_rows].time);
            QuadPwrBudget[counter_cols * nb_rows + counter_rows].value =
                atof(col); // TODO Check this! is it nb_* or simulation_elements..?
            // printf("%d/%d: %f\n", counter_rows, counter_cols, QuadPwrBudget[counter_cols*nb_rows
            // + counter_rows].value);
    ...
    if (nb_cols != simulation.nb_cores) {
        printf("[CSV] Error: Binding Matrix file has a different number of columns (%d) wrt the "
               "number of cores in the Model (%d)\r\n",
    ...
    BindMatrix =
        (input_data_bm_t *)malloc(sizeof(input_data_bm_t) * nb_cols *
                                  nb_rows); // TODO Check this! is it nb_* or simulation_elements..?
    ...
    while (row = CsvReadNextRow(handle)) {
        ...
        while (col = CsvReadNextCol(row, handle)) {
            /* col = CSV col string */
            BindMatrix[counter_cols * nb_rows + counter_rows].time =
                time; // TODO Check this! is it nb_* or simulation_elements..?
            // printf("%d/%d: %d\n", counter_rows, counter_cols, BindMatrix[counter_cols*nb_rows +
            // counter_rows].time);
            BindMatrix[counter_cols * nb_rows + counter_rows].value =
                atoi(col); // TODO Check this! is it nb_* or simulation_elements..?
            // printf("%d/%d: %d\n", counter_rows, counter_cols, BindMatrix[counter_cols*nb_rows +
            // counter_rows].value);
...
const input_data_t BoardPwrBudget[N_PW_BUDGET_SEQ] = { {.value=120.0f, .time=1000}, {.value=75.0f,
.time=1000}, {.value=40.0f, .time=1000}, {.value=90.0f, .time=1000}, {.value=120.0f, .time=12050},
{.value=0, .time=0}
                                                                                                        };
*/
/*
const input_data_bm_t BindMatrix[N_BINDING_MAT_SEQ] = { {.value[0]=0, .value[1]=0, .value[2]=0,
.value[3]=0, .value[4]=0, .value[5]=0, .value[6]=0, .value[7]=0, .value[8]=0, .value[9]=0,
.value[10]=0, .value[11]=0, .value[12]=0, .value[13]=0, .value[14]=0, .value[15]=0, .value[16]=0,
.value[17]=0, .value[18]=0, .value[19]=0, .value[20]=0, .value[21]=0, .value[22]=0, .value[23]=0,
.value[24]=0, .value[25]=0, .value[26]=0, .value[27]=0, .value[28]=0, .value[29]=0, .value[30]=0,
.value[31]=0, .value[32]=0, .value[33]=0, .value[34]=0, .value[35]=0, .time=5500}, //1
