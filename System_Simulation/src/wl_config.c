...
/*
wl_block_t init_wl_block[12] = { {1,50 },{4,100 },{2,50}, {3,50},{2,50},{0,50},{4,100 },{3,50 },{0,50 },{3,50 },{2,100 },{0,0} }; //1250
//
...
//
wl_block_t mix1_wl_block[6] = { { 1,100 }, {2, 250}, {0, 475}, {1, 400}, {3, 500}, { 0,0 } }; //15000
//
wl_block_t mix2_wl_block[6] = { { 3,100 }, {1, 425}, {3, 450}, {2, 250}, {1, 500}, { 0,0 } }; //15000
//
wl_block_t mix3_wl_block[6] = { { 1,400 }, {0, 175}, {4, 250}, {1, 600}, {0, 300}, { 0,0 } }; //15000
//
...
vMacroblock_t openmpi1[18] = {
	...
	{ init_wl_block, 2 }, { memory_wl_block, 2}, { open_mpi_wl_block1, 45}, { memory_wl_block, 4},
	{init_wl_block, 2 }, { memory_wl_block, 1}, { init_wl_block, 2 },
	{ init_wl_block, 2 }, { memory_wl_block, 2}, { open_mpi_wl_block1, 45}, { memory_wl_block, 4},
	 { NULL, 0 } }; //1.005.000

vMacroblock_t openmpi2[21] = { { init_wl_block, 4}, { memory_wl_block, 4}, { high_compute_wl_block, 1}, 
	{init_wl_block, 2 }, { memory_wl_block, 2}, { init_wl_block, 2 },
	{ init_wl_block, 2 }, { memory_wl_block, 2}, { open_mpi_wl_block2, 45}, { memory_wl_block, 4},
	{init_wl_block, 2 }, { memory_wl_block, 1}, { init_wl_block, 2 }, {init_wl_block, 2 }, { memory_wl_block, 76}, { init_wl_block, 2},
	{ init_wl_block, 2 }, { memory_wl_block, 2}, { open_mpi_wl_block1, 45}, { memory_wl_block, 4},
	 { NULL, 0 } }; //1.105.000
*/
...
const vSeqblock_t Workload[N_EPI_CORE][N_WL_SEQ] = { { {.block_name=init_wl, .times=1}, 
												  {.block_name=complete_wl, .times=100} },
//1
											    { {.block_name=init_wl, .times=1}, 
												  {.block_name=complete_wl, .times=10} },
//2
												{ {.block_name=init_wl, .times=1}, 
												  {.block_name=complete_wl, .times=10} },
//3
...
count_times++;
	...
	for (int core = 0; core<N_EPI_CORE; core++)
	{
		...
		*/
		...
		{
			//Take infos
			float cycles = mul*macroblock_address[macroblock_counter[core]].block_name[block_counter[core]].cycles;
			//if (core==0){printf("%f\n\r", cycles);}
			...
			if (cycles <= 0)
			{
				...
				//take new values
				cycles = mul*macroblock_address[macroblock_counter[core]].block_name[block_counter[core]].cycles;
				//if (core==0){printf("%f\n\r", cycles);}
			}
			...
			{
				cycles = cycles/100.0f*90.0f + cycles/mem_cycles/100*cycles_per_step*10.0f; 
			}
			else if ((wl_state == 2))
			{
			//printf("cycles: %f, M_Q: %f, m_c: %f, cps: %f, res: %f\n\n\r", cycles, WL_MEMORY_QUOTA, mem_cycles, cycles_per_step, 
			//				cycles/100*(100.0f-WL_MEMORY_QUOTA) + cycles/mem_cycles/100*cycles_per_step*WL_MEMORY_QUOTA);
				cycles = cycles/100*(100.0f-WL_MEMORY_QUOTA) + cycles/mem_cycles/100*cycles_per_step*WL_MEMORY_QUOTA; 
			}
			else if ((wl_state == 3))
			{
		...
		//TO consider memory
		//0
		simulation.elements[core].core_config.workload[0] += (float)accum_cycles[0] / (float)cycles_per_step;
		//1
		simulation.elements[core].core_config.workload[WL_MEMORY_STATE] += (float)accum_cycles[1] * 10.0f/100.0f / (float)cycles_per_step;
		simulation.elements[core].core_config.workload[1] += (float)accum_cycles[1] * 90.0f/100.0f / (float)cycles_per_step;
		//2
		//printf("cycles: %f, M_Q: %f, m_c: %f, cps: %f, res: %f\n\n\r", cycles, WL_MEMORY_QUOTA, mem_cycles, cycles_per_step, 
		//				cycles/100*(100.0f-WL_MEMORY_QUOTA) + cycles/mem_cycles/100*cycles_per_step*WL_MEMORY_QUOTA);
		simulation.elements[core].core_config.workload[WL_MEMORY_STATE] += (float)accum_cycles[2] * WL_MEMORY_QUOTA/100.0f / (float)cycles_per_step;
		simulation.elements[core].core_config.workload[2] += (float)accum_cycles[2] * (100.0f-WL_MEMORY_QUOTA)/100.0f / (float)cycles_per_step;
		//3
		simulation.elements[core].core_config.workload[WL_MEMORY_STATE] += (float)accum_cycles[3] * 15.0f/100.0f / (float)cycles_per_step;
		simulation.elements[core].core_config.workload[3] += (float)accum_cycles[3] * 85.0f/100.0f / (float)cycles_per_step;
		//4
		simulation.elements[core].core_config.workload[4] += (float)accum_cycles[4] / (float)cycles_per_step;
