...
float constant_parameter_deviation[36] = {
    102.829826, 103.376648, 103.92144,  105.575813, 102.984085,
    103.015053, 104.723274, 103.396263, 104.62513,

    104.85408,  103.248093, 104.179611, 105.452026, 103.569344,
    106.145004, 103.760078, 104.386665, 103.893303,

    105.23027,  103.896332, 104.006592, 102.723488, 105.160606,
    102.424133, 102.297165, 105.767616, 104.434547,

    103.279343, 105.896286, 103.253319, 104.001114, 102.666077,
    103.194771, 105.131111, 102.493217, 104.080215};
...
int initialize_simstruct(char *filepath) {
    ...
    cJSON_ArrayForEach(element, jsonarray) {
        ...
        // INTER-WAFER
        chiplet_element =
            cJSON_GetObjectItemCaseSensitive(element, "inter-wafer_silicon_variation");
        // TODO: check?
        ...
        if (cJSON_IsNumber(uncert_element)) {
            ...
            printf("[JSON] Checking component %d inter-wafer gaussian mean\n\r",
                   chiplet_model_ptr[index].cid);
#endif
            chiplet_model_ptr[index].inter_wafer_silicon_var.mean = uncert_element->valuedouble;
        } else {
            // TODO
            printf(
                "[JSON] Component %d inter-wafer gaussian mean is not a number, using DEFAULT\n\r",
                chiplet_model_ptr[index].cid);
            // exit(1);
        ...
        if (cJSON_IsNumber(uncert_element)) {
            ...
            // TODO
            printf("[JSON] Component %d inter-wafer gaussian variance is not a number, using "
                   "DEFAULT\n\r",
                   chiplet_model_ptr[index].cid);
            // exit(1);
        ...
        if (cJSON_IsNumber((uncert_element != NULL) ? (uncert_element)->child : NULL)) {
            ...
            printf("[JSON] Checking component %d inter-wafer variance lower limit\n\r",
                   chiplet_model_ptr[index].cid);
#endif
            chiplet_model_ptr[index].inter_wafer_silicon_var.variance_limits_perc[0] =
                (uncert_element)->child->valuedouble;

            // second point, only if first exist, otherwise we have Seg Fault
            if (cJSON_IsNumber(((uncert_element)->child != NULL) ? (uncert_element)->child->next
                                                                 : NULL)) {
#ifdef JSON_DEBUG
                printf("[JSON] Checking component %d inter-wafer variance upper limit\n\r",
                       chiplet_model_ptr[index].cid);
#endif
                chiplet_model_ptr[index].inter_wafer_silicon_var.variance_limits_perc[1] =
                    uncert_element->child->next->valuedouble;
            } else {
                // TODO
                printf("[JSON] Component %d inter-wafer variance upper limit is not a number, "
                       "using DEFAULT\n\r",
                       chiplet_model_ptr[index].cid);
                // exit(1);
                chiplet_model_ptr[index].inter_wafer_silicon_var.variance_limits_perc[1] =
                    1000; // 1000 = 100,000% = no limit
            }
        } else {
            // TODO
            printf("[JSON] Component %d inter-wafer variance lower limit is not a number, using "
                   "DEFAULT\n\r",
                   chiplet_model_ptr[index].cid);
            // exit(1);
            chiplet_model_ptr[index].inter_wafer_silicon_var.variance_limits_perc[0] =
        ...
        // 4
        uncert_element =
            cJSON_GetObjectItemCaseSensitive(chiplet_element, "3sigma=30:variation_perc");
        if (cJSON_IsNumber(uncert_element)) {
#ifdef JSON_DEBUG
            printf("[JSON] Checking component %d 3sigma=30:variation_perc\n\r",
                   chiplet_model_ptr[index].cid);
#endif
            chiplet_model_ptr[index].inter_wafer_silicon_var.three_sigma_prop_perc =
                uncert_element->valuedouble;
        } else {
            // TODO
            printf(
                "[JSON] Component %d 3sigma=30:variation_perc is not a number, using DEFAULT\n\r",
                chiplet_model_ptr[index].cid);
            // exit(1);
            chiplet_model_ptr[index].inter_wafer_silicon_var.three_sigma_prop_perc = 1.0;
        }

        // The Gaussian noise silicon variation it is normalized over a 3*sigma=30 --> variance =
        // 100
        ...
        do {
            ...
            generateGaussianNoise(&gaussian_var, NULL,
                                  chiplet_model_ptr[index].inter_wafer_silicon_var.mean, sigma);
            res = (gaussian_var *
                   chiplet_model_ptr[index].inter_wafer_silicon_var.three_sigma_prop_perc) /
                  30.0f;
        ...
        // here I repeat instead of just doing saturation, because If I saturate, I would violate
        // the normal distribution
        //   given that at the boundaries I will have a spike = the integral of the gaussian
        //   distribution from limit to infinity.
    ...
    cJSON_ArrayForEach(element, jsonarray) {
        ...
        */
        ...
        if (total_component_number < core_components[index].wl_states) {
            ...
            printf(
                "[JSON] Attention,there are more dyn coeff values then WL_STATES dim in core\n\r");
        }
        int index2 = 0;
        core_components[index].dyn_pow_cpu_coeff = (float *)malloc(
            sizeof(float) * total_component_number); // TODO: or core_components[index].wl_states
        cJSON_ArrayForEach(dyn_element, core_element) {
            if (cJSON_IsNumber(dyn_element) && (dyn_element->valuedouble >= 0.0f)) {
#ifdef JSON_DEBUG
                printf("[JSON] Checking dyn_coeff %d \"%lf\"\n\r", index2,
                       dyn_element->valuedouble);
#endif
            ...
            index2++;
            if (index2 >= core_components[index].wl_states) break;
        }
        ...
        if (cJSON_IsNumber(uncert_element)) {
            ...
            printf("[JSON] Checking component %d intra-die gaussian variance \n\r",
                   core_components[index].cid);
#endif
            core_components[index].uncert.intra_die_silicon_var.variance =
                uncert_element->valuedouble;
        } else {
            // TODO
            printf("[JSON] Component %d intra-die gaussian variance is not a number, using "
                   "DEFAULT\n\r",
                   core_components[index].cid);
            // exit(1);
        ...
        if (cJSON_IsNumber((uncert_element != NULL) ? (uncert_element)->child : NULL)) {
            ...
            printf("[JSON] Checking component %d intra-die variance lower limit\n\r",
                   core_components[index].cid);
#endif
            core_components[index].uncert.intra_die_silicon_var.variance_limits_perc[0] =
                (uncert_element)->child->valuedouble;
            ...
            if (cJSON_IsNumber(((uncert_element)->child != NULL) ? (uncert_element)->child->next
                ...
                printf("[JSON] Checking component %d intra-die variance upper limit\n\r",
                       core_components[index].cid);
#endif
                core_components[index].uncert.intra_die_silicon_var.variance_limits_perc[1] =
                    (uncert_element)->child->next->valuedouble;
            } else {
                // TODO
                printf("[JSON] Component %d intra-die variance upper limit is not a number, using "
                       "DEFAULT\n\r",
                       core_components[index].cid);
                // exit(1);
                core_components[index].uncert.intra_die_silicon_var.variance_limits_perc[1] =
            ...
            // TODO
            printf("[JSON] Component %d intra-die variance lower limit is not a number, using "
                   "DEFAULT\n\r",
                   core_components[index].cid);
            // exit(1);
            core_components[index].uncert.intra_die_silicon_var.variance_limits_perc[0] =
        ...
        if (cJSON_IsNumber(uncert_element)) {
            ...
            printf("[JSON] Checking component %d 3sigma=30:variation_perc\n\r",
                   core_components[index].cid);
#endif
            core_components[index].uncert.intra_die_silicon_var.three_sigma_prop_perc =
                uncert_element->valuedouble;
        } else {
            // TODO
            printf(
...
#endif
            if (strlen(uncert_element->valuestring) <
                STRING_DIM_NOISE_TYPE) // strictly because string terminator
                strncpy(core_components[index].uncert.temperature_noise.type,
                        uncert_element->valuestring, STRING_DIM_NOISE_TYPE);
            else {
                printf("[JSON] Component %d temperature noise type is exceeding maximum string dim "
                       "of %d\n\r\t---so probably incorrect type is selected, WHITE as default\n\r",
                       core_components[index].cid, STRING_DIM_NOISE_TYPE);
                strncpy(core_components[index].uncert.temperature_noise.type, noise_white,
                        STRING_DIM_NOISE_TYPE);
            }
        } else {
            // TODO
...
// Check of the correct string will be done after
            ...
            if (strlen(uncert_element->valuestring) <
                STRING_DIM_NOISE_TYPE) // strictly because string terminator
                strncpy(core_components[index].uncert.workload_noise.type,
                        uncert_element->valuestring, STRING_DIM_NOISE_TYPE);
            else {
                printf("[JSON] Component %d workload noise type is exceeding maximum string dim of "
                       "%d\n\r\t---so probably incorrect type is selected, WHITE as default\n\r",
                       core_components[index].cid, STRING_DIM_NOISE_TYPE);
                strncpy(core_components[index].uncert.workload_noise.type, noise_white,
                        STRING_DIM_NOISE_TYPE);
            }
        } else {
            // TODO
    ...
    struct common_config_st *common_components =
        (struct common_config_st *)malloc(sizeof(struct common_config_st) * total_component_number);
...
// Check of the correct string will be done after
#endif
        if ((!strcmp(element->valuestring, "yes")) || (!strcmp(element->valuestring, "Yes")) ||
            (!strcmp(element->valuestring, "y")) || (!strcmp(element->valuestring, "Y"))) {
            USE_ALL_DOMAINS = 1;
        } else {
            USE_ALL_DOMAINS = 0;
        }
...
// Check of the correct string will be done after
    ...
    struct chiplet_link_t *chiplet_link_ptr =
        malloc(sizeof(struct chiplet_link_t) * nb_chiplet_links);
    index = 0;
    cJSON_ArrayForEach(element, chiplets) {
    ...
    simulation.elements = malloc(sizeof(struct element_st) * simulation.nb_elements);
    ...
    cJSON_ArrayForEach(element, component) {
        ...
        for (int i = 0; i < number; i++) {
            ...
            simulation.elements[sim_element_counter].id =
                (uint32_t)i; // TOFO fix id. Give it a proper meaning.
            ...
            if (cid < JSON_CORE_CID) {
                ...
                for (int j = 0; j < nb_core_components; j++) {
                    if (core_components[j].cid ==
                        simulation.elements[sim_element_counter].core_config.cid) {
                        // The Gaussian noise silicon variation it is normalized over a 3*sigma=30
                        // --> variance = 100
                        // TODO: to improve this, I may make a PAPER study on how the inter-die
                        // discrepancies are distributed
                        //       and instead of making it random, I will grab a ncore string of
                        //       gaussian distributed values and then distribute imperfection based
                        //       on physic and research
                        float gaussian_var, res;
                        do {
                            float sigma = (float)sqrt(
                                core_components[j].uncert.intra_die_silicon_var.variance);
                            generateGaussianNoise(
                                &gaussian_var, NULL,
                                core_components[j].uncert.intra_die_silicon_var.mean, sigma);
                            res = (gaussian_var *
                                   core_components[j]
                                       .uncert.intra_die_silicon_var.three_sigma_prop_perc) /
                                  30.0f;
                        } while (
                            (res < core_components[j]
                                       .uncert.intra_die_silicon_var.variance_limits_perc[0]) ||
                            (res > core_components[j]
                                       .uncert.intra_die_silicon_var.variance_limits_perc[1]));
                        // here I repeat instead of just doing saturation, because If I saturate, I
                        // would violate the normal distribution
                        //   given that at the boundaries I will have a spike = the integral of the
                        //   gaussian distribution from limit to infinity.

                        if (!USE_PARAMETER_DEVIATION) {
                            simulation.elements[sim_element_counter].core_config.leak_vdd =
                                core_components[j].leak_vdd;
                            simulation.elements[sim_element_counter].core_config.leak_temp =
                                core_components[j].leak_temp;
                            simulation.elements[sim_element_counter].core_config.leak_process =
                                core_components[j].leak_process;
                            simulation.elements[sim_element_counter].core_config.wl_states =
                                core_components[j].wl_states;
                            ...
                            for (int k = 0; k < core_components[j].wl_states; k++)
                                simulation.elements[sim_element_counter]
                                    .core_config.dyn_pow_cpu_coeff[k] =
                            ...
                            if (!USE_CONSTANT_DEVIATION) {
                                ...
                                printf("[JSON] \t core %d/%d variation: %f -- total: %f\n\r", i,
                                       core_components[j].cid, res,
                                       (1.0f + chiplet_silicon_var + res) * 100.0f);
#endif
                                simulation.elements[sim_element_counter].core_config.leak_vdd =
                                    core_components[j].leak_vdd *
                                    (1.0f + chiplet_silicon_var + res);
                                simulation.elements[sim_element_counter].core_config.leak_temp =
                                    core_components[j].leak_temp *
                                    (1.0f + chiplet_silicon_var + res);
                                simulation.elements[sim_element_counter].core_config.leak_process =
                            ...
                            } else // not USE_CONSTANT_DEVIATION
                            {
                                ...
                                printf("[JSON] \t core %d/%d variation: %f -- total: %f\n\r", i,
                                       core_components[j].cid, res,
                                       constant_parameter_deviation[sim_element_counter]);
#endif
                                simulation.elements[sim_element_counter].core_config.leak_vdd =
                                    core_components[j].leak_vdd *
                                    constant_parameter_deviation[sim_element_counter] / 100.0f;
                                simulation.elements[sim_element_counter].core_config.leak_temp =
                                    core_components[j].leak_temp *
                                    constant_parameter_deviation[sim_element_counter] / 100.0f;
                                simulation.elements[sim_element_counter].core_config.leak_process =
                        ...
                        // compute_power
                        simulation.elements[sim_element_counter].compute_power =
                            compute_core_power_cid2; // TODO qua è hardcoded
...
float compute_core_power_cid1(struct element_st *self, float freq, float voltage, float temp,
                              float process, float *workload_perc) {
    float p_stat = 0.0;
    float p_dyn = 0.0;
    ...
    p_stat = ((voltage * (voltage / self->core_config.leak_vdd) * 1000.0f) +
              (tempC > 85.0f
                   ? ((tempC * self->core_config.leak_temp) - (85.0f * self->core_config.leak_temp))
                   : 0.0f) +
              process * self->core_config.leak_process) /
             1000.0f;
...
float compute_core_power_cid2(struct element_st *self, float freq, float voltage, float temp,
    ...
    float tempC = temp - 273.15f;
    // computing the static power using precomputed coefficients called leak_*
    p_stat = ((voltage * self->core_config.leak_vdd) + process * self->core_config.leak_process) /
             1000.0f;

    // Themal relation of the leakage, including thermal inversion
    p_stat *= exp((4507.748f * voltage + 29.903f * (tempC > 125.0f ? 125.0 : tempC) - 6033.035f) /
                  1000.0f);
    // printf("p: %f, exp: %f\n", p_stat, exp( (4507.748f * voltage + 29.903f * tempC - 6033.035f) /
    // 1000.0f ));
