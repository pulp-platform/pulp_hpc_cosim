...
void init_quanta(void) {
    head_quantum =
        (struct quantum_st **)malloc(sizeof(struct quantum_st *) * simulation.nb_elements);
    current_quantum =
        (struct quantum_st **)malloc(sizeof(struct quantum_st *) * simulation.nb_elements);
    last_quantum =
        (struct quantum_st **)malloc(sizeof(struct quantum_st *) * simulation.nb_elements);
...
int generate_wl(float *owl, float ifreq, int elem_counter, struct element_st *element) {
    ...
    while (total_cycles < float_apprx_coeff * cycles_per_step) {
        ...
        if ((current_quantum[elem_counter]->us_wait_time > 0) &&
            (current_quantum[elem_counter]->us_wait_time -
                 current_quantum[elem_counter]->us_waited_time >=
             0)) {
            // Convert cycles in time
            float time_us = (cycles_per_step - total_cycles) / (ifreq * 1000.0f);
            time_us = fminf(time_us, current_quantum[elem_counter]->us_wait_time -
                                         current_quantum[elem_counter]->us_waited_time);
            /*
            if (elem_counter == 0){
                printf("** Time: %f\n\r", time_us);
            ...
            */
            ...
            // Check for numbers less than 1 (//TBC) - Saturate to 1 to avoid being stuck in the
            // while loop
        ...
        float cycles = fminf(
            ((float)(current_quantum[elem_counter]->dim - current_quantum[elem_counter]->consumed) *
             current_quantum[elem_counter]->cpi_freq),
            (cycles_per_step - total_cycles));
        // Check for numbers less than 1 (//TBC) - Saturate to 1 to avoid being stuck in the while
        // loop
        //      Otherwise saturate to 0 to avoid nagatives
        ...
        total_cycles += cycles;
        current_quantum[elem_counter]->consumed +=
            (int64_t)ceil(cycles / current_quantum[elem_counter]->cpi_freq); // TBC: ceil

        /*
        if (elem_counter == 0){
        printf("Tot Cycl in while: %f, consumed: %ld, added2cons: %d\n\r", total_cycles,
                current_quantum[elem_counter]->consumed,
        (int)(ceil(cycles/current_quantum[elem_counter]->cpi_freq))); for(int s=0;
        s<get_component_states_number(&simulation.elements[elem_counter]); s++) printf("[%d]=%f , ",
        s, accum_instr[s]); printf("\n\r");
        }
        */
...
static inline int move_current_pointer(int elem_counter) {
    ...
    if ((current_quantum[elem_counter]->consumed - current_quantum[elem_counter]->dim >= 0) &&
        (current_quantum[elem_counter]->us_waited_time >=
         float_apprx_coeff * current_quantum[elem_counter]->us_wait_time)) {
