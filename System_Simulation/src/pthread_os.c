    ...
    while (*gn_run_simulation) {
        ...
        if (!(*gn_pause_simulation)) {
            ...
            for (int core = 0; core < simulation.nb_elements; core++) {
                ...
                if (target_frequency_time[core] >=
                    TargetFrequency[core * N_FREQ_SEQ + target_frequency_counter[core]].time *
                        freq_time_multiplier) {
                    target_frequency_time[core] = 0;
                    target_frequency_counter[core]++;
                    if ((TargetFrequency[core * N_FREQ_SEQ + target_frequency_counter[core]].time <=
                         0) ||
                        (target_frequency_counter[core] >= N_FREQ_SEQ)) {
...
/* SCMI*/
                    ...
                    otpc_core_target_freq[core] =
                        TargetFrequency[core * N_FREQ_SEQ + target_frequency_counter[core]].value;
#endif
                }
...
/* SCMI*/
            ...
            for (int quad = 0; quad < simulation.nb_power_domains; quad++) {
                ...
                if (pwr_budget_quad_time[quad] >=
                    QuadPwrBudget[quad * N_PW_BUDGET_SEQ + pwr_budget_quad_counter[quad]].time *
                        quad_time_multiplier) {
                    pwr_budget_quad_time[quad] = 0;
                    pwr_budget_quad_counter[quad]++;
                    if ((QuadPwrBudget[quad * N_PW_BUDGET_SEQ + pwr_budget_quad_counter[quad]]
                             .time <= 0) ||
                        (pwr_budget_quad_counter[quad] >= N_PW_BUDGET_SEQ)) {
                        pwr_budget_quad_counter[quad] = 0;
                    }

                    otpc_domain_pw_budget[quad + 1] =
                        QuadPwrBudget[quad * N_PW_BUDGET_SEQ + pwr_budget_quad_counter[quad]].value;
                }
            ...
            if (pwr_budget_board_time >=
                BoardPwrBudget[pwr_budget_board_counter].time * board_time_multiplier) {
                pwr_budget_board_time = 0;
                pwr_budget_board_counter++;
                if ((BoardPwrBudget[pwr_budget_board_counter].time <= 0) ||
                    (pwr_budget_board_counter >= N_PW_BUDGET_SEQ)) {
                    pwr_budget_board_counter = 0;
                }
            ...
            if (bind_mat_time >=
                BindMatrix[0 * N_BINDING_MAT_SEQ + bind_mat_counter].time * bind_time_multiplier) {
                bind_mat_time = 0;
                bind_mat_counter++;
                if ((BindMatrix[0 * N_BINDING_MAT_SEQ + bind_mat_counter].time <= 0) ||
                    (bind_mat_counter >= N_BINDING_MAT_SEQ)) {
                    bind_mat_counter = 0;
                }

                for (int core = 0; core < simulation.nb_cores; core++) {
                    otpc_core_bindings[core] =
                        BindMatrix[core * N_BINDING_MAT_SEQ + bind_mat_counter].value;
                }
            }
